...
def visualize_results(
    ...
    human_rois: dict[str, tuple[int, int, int]] | None = None,
) -> None:
    """
    ...
    if human_rois:
        for roi_name, center in human_rois.items():
            ...
            ax1.scatter(
                x,
                y,
                c=colors.get(roi_name, "green"),
                s=100,
                marker="x",
                label=f"Human {roi_name}",
                linewidths=2,
            )
    ...
    for _roi_name, center in detector.selected_rois.items():
        x, y, _ = center
        circle = Circle((x, y), roi_radius_xy, linewidth=2, edgecolor="white", facecolor="none")
    ...
    ax3.imshow(
        fa_zoom, cmap=fa_cmap, origin="lower", vmin=0, vmax=1, extent=[x_min, x_max, y_min, y_max]
    )
